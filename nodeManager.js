...
let NODECONFIG = {...loadNodeConfig(),...fs.existsSync("CustomNodes/config.json")?JSON.parse(fs.readFileSync("CustomNodes/config.json")):{}};

// Initialize Node classes
for (let i in NODECONFIG) {
    try {
        if(NODECONFIG[i].gen){
            NODECONFIG[i].func = require("./CustomNodes/" + NODECONFIG[i].func + ".js");
        }
        else{
